...
public class Attachjar {
	...
	public static void runProcess(String input, String option) {
		try {
			...
			switch (extension) {
			case "jar":
				builder = new ProcessBuilder("java", "-jar", f.getAbsolutePath() + option);
				builder.directory(new File(System.getProperty("java.home") + "\\bin\\"));
				break;
			case "class":
				builder = new ProcessBuilder("java", f.getAbsolutePath() + option);
				builder.directory(new File(System.getProperty("java.home") + "\\bin\\"));
				break;
			default:
				builder = new ProcessBuilder(f.getAbsolutePath() + option);
				break;
			}

			Process process = builder.start();
			System.out.println(runtime + " / " + all + " ("
					+ new DecimalFormat("0.#").format(((float) runtime / (float) all) * 100) + "%)..\r\n");
			...
			if (err.length() > 0)
				...
				successcount++;
			if (generateErrLog && err.length() > 0)
				generateLog(err);
